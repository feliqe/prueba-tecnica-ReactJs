// funcion para tomar JSON de la pagina
export async function getPeople(page) {

    try {
        const response = await fetch(`https://swapi.dev/api/people/?page=${page}`);
        // validamos si la pagina responde bien
        if(!response.ok){
...
// funcion para extaer la data de un id evnviado
export async function getCharacter(id = 1){
    try {
        ...
        const data = await response.json();
        // data tiene toda la informacion del personaje
        return data;
    } catch (err) {
        _throwSpecificError(err);
    }
...
//constructor de error por tipos
